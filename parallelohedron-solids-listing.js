...
function standardizeCameras(modelData) {
	...
	// Any model could be the one that sets the default camera if the "P=" queryparam is used.
	...
	for(let scene of modelData.scenes) {
		// scene views are not used by the parallelohedron solids app, but we'll standardize their cameras too since we're here
...
function getFaceSceneSnapshots(modelData) {
	// Get a list of facescene(s) of all models that use the selected psolid's URL.
	// There may be only one facescene, but there may be more than one.
